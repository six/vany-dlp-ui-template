...
import { IpfsUploadResponse } from "./useDataUpload";
...
export function useContributionFlow() {
  ...
  const handleContributeData = async (
    ...
    try {
      ...
      updateContributionData({
        contributionId: uploadResult.ipfsHash,
        encryptedUrl: uploadResult.ipfsUrl,
        transactionReceipt: {
  ...
  const executeSignMessageStep = async (): Promise<string | undefined> => {
    try {
      ...
      console.log("Signed message (used for encryption):", signature);
      return signature;
    } catch (signError) {
  ...
  const executeUploadDataStep = async (
    ...
    const uploadResult = await uploadData(userInfo, signature, file, walletAddress);
    if (!uploadResult) {
      setError("Failed to upload data to IPFS");
      return null;
    }
    setShareUrl(uploadResult.ipfsUrl);
  ...
  const executeBlockchainRegistrationStep = async (
    uploadResult: IpfsUploadResponse,
    signature: string
  ) => {
    ...
    const txReceipt = await addFile(uploadResult.ipfsUrl, encryptedKey);
  ...
  const updateContributionData = (newData: Partial<ContributionData>) => {
    setContributionData((prev) => ({ ...prev, ...newData } as ContributionData));
  };
